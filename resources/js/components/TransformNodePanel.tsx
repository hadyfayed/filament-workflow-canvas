...
export const TransformNodePanel: FC<TransformNodePanelProps> = ({ config, updateConfig }) => {
    ...
    return (
        <div className="space-y-4">
            <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-200 mb-1">Transform Type</label>
                <select
                    ...
                    {mappings.map((mapping, index) => (
                        <div key={index} className="p-3 bg-gray-50 dark:bg-gray-800/50 rounded-md border border-gray-200 dark:border-gray-700 space-y-2">
                             <div className="flex justify-between items-center">
                                <p className="text-sm font-medium text-gray-600 dark:text-gray-300">Mapping #{index + 1}</p>
                                <button onClick={() => removeMapping(index)} className="text-gray-400 hover:text-red-500 dark:hover:text-red-400">
            ...
            {config.transform_type === 'javascript' && (
                <div>
                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-200 mb-1">JavaScript Code</label>
                    <textarea
                        value={config.code || ''}
                        ...
                        placeholder={'// Access input with `data`\n// Return new data object\nreturn { ...data, new_field: "value" }'}
                    />
                </div>
            )}
            ...
            {config.transform_type === 'template' && (
                <div>
                    ...
                    />
                     <p className="text-xs text-gray-500 mt-1">Use Mustache-style templates with `{'{{ field }}'}`.</p>
                </div>
            )}
        </div>
    );
