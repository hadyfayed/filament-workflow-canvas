...
export const BaseNode: FC<BaseNodeProps> = ({
  ...
  const getBorderClasses = () => {
    ...
    if (selected) {
      switch (color) {
        case 'primary':
          return 'border-primary-500 dark:border-primary-400 ring-2 ring-primary-500/50';
        case 'warning':
          return 'border-warning-500 dark:border-warning-400 ring-2 ring-warning-500/50';
        case 'purple':
          return 'border-purple-500 dark:border-purple-400 ring-2 ring-purple-500/50';
        case 'success':
          return 'border-success-500 dark:border-success-400 ring-2 ring-success-500/50';
      }
    }
  ...
  return (
    <div
      className={`relative px-4 py-3 rounded-lg bg-white dark:bg-gray-800 border-2 transition-all duration-200 min-w-[220px] shadow-sm hover:shadow-md ${getBorderClasses()}`}
    >
      {hasInput && (
        <Handle
          type="target"
      ...
      <div className="flex items-center space-x-3">
        <div
          className={`w-10 h-10 ${getIconBgClasses()} rounded-lg flex items-center justify-center`}
        >
          {icon}
        </div>
        ...
        {data.hasError && <ExclamationCircleIcon className="w-6 h-6 text-danger-500" />}
      </div>
      {data.description && (
